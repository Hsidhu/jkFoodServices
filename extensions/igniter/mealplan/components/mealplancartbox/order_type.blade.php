<!-- copied from local component  control-->
    ...
    @if (count($locationOrderTypes) <= $__SELF__->property('maxOrderTypeButtons', 2))
        ...
        >
            @foreach($locationOrderTypes as $orderType)
                ...
                >@partial('@order_type_info', ['orderType' => $orderType])</label>
            @endforeach
        </div>
    @else
        <div
        ...
        >
            <button
                ...
                aria-expanded="false"
            >
                @partial('@order_type_info', ['orderType' => $location->getOrderType()])
            </button>
            <div class="dropdown-menu w-100" aria-labelledby="dropdownMenuButton">
                @foreach($locationOrderTypes as $orderType)
                    ...
                    >
                        @partial('@order_type_info', ['orderType' => $orderType])
                    </a>
                @endforeach
            </div>
        </div>
